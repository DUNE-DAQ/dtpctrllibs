...
//-----------------------------------------------------------------------------
void
DTPController::init(const data_t& /* init_data */)
{
...
DTPController::do_configure(const data_t& args)
{
  ...
  for (uint i_link = 0; i_link < n_links; ++i_link) {
    TLOG_DEBUG(TLVL_INFO) << get_name() << ": setting threshold for link processor " << i_link << " to "
                          << m_dtp_cfg.threshold;
  ...
  if (masks_size == n_links * n_streams) {
    for (uint i_link = 0; i_link < n_links; i_link++) {
      for (uint i_stream = 0; i_stream < n_streams; i_stream++) {
        ...
        TLOG_DEBUG(TLVL_INFO) << get_name() << ": setting masks for link " << i_link << " stream " << i_stream << " to "
                              << std::hex << mask;
...
DTPController::do_start(const data_t& /* args */)
{
  ...
  for (uint i_link = 0; i_link < n_links; ++i_link) {
    for (uint i_stream = 0; i_stream < n_streams; ++i_stream) {
      TLOG_DEBUG(TLVL_INFO) << get_name() << ": disabling pedestal capture for link " << i_link << " stream "
                            << i_stream;
...
//-----------------------------------------------------------------------------
void
DTPController::do_scrap(const data_t& /* args */)
{
